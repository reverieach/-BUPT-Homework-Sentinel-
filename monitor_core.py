...
def _make_session(settings: Settings) -> requests.Session:
...
def _course_list_url(settings: Settings) -> str:
    return f"{settings.api_base_url.rstrip('/')}/{settings.course_list_endpoint.lstrip('/')}"
...
def _load_course_map(settings: Settings) -> dict[str, str]:
    """Load cached activityId->courseName map. Returns empty if stale (>1 day)."""
    path = settings.course_map_file
    if not path.exists():
        return {}
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
        ...
        if updated:
            ...
            age = (_now_local() - dt).total_seconds()
            if age > 86400:
                return {}
...
def _save_course_map(settings: Settings, mapping: dict[str, str]) -> None:
    payload = dict(mapping)
    payload["_updated"] = _now_local().isoformat()
    path = settings.course_map_file
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")


def _detect_current_term(
    session: requests.Session,
    headers: dict[str, str],
    settings: Settings,
    user_id: str,
) -> str:
    """Auto-detect current termId by fetching a small course list and picking the most common term."""
    try:
        resp = session.get(
            _course_list_url(settings),
            headers=headers,
            params={"userId": user_id, "size": 20, "current": 1},
        ...
        # Count termId occurrences — the most recent page is sorted by latest,
        # so the most frequent termId on page 1 is the current semester.
        from collections import Counter
        term_counts: Counter[str] = Counter()
        for rec in records:
            if not isinstance(rec, dict):
                continue
            tid = str(rec.get("termId", "")).strip()
            if tid and tid != "-1":
                term_counts[tid] += 1
...
def fetch_course_map(settings: Settings) -> dict[str, str]:
    """Build activityId->courseName mapping by scanning all enrolled courses.

    Returns cached version if fresh (<1 day old).
    """
    cached = _load_course_map(settings)
    if cached:
        print(f"[ok] course map loaded from cache ({len(cached)} entries)")
        return cached

    headers = _load_headers(settings)
    user_id = _infer_user_id_from_headers_or_token(settings, headers)
    if not user_id:
        print("[warn] cannot build course map: no user_id")
        return {}

    session = _make_session(settings)

    # Step 1: fetch all enrolled courses
    print("[step] fetching course list...")
    try:
        params: dict[str, Any] = {"userId": user_id, "size": 200, "current": 1}
        term_id = settings.term_id

        # Auto-detect current term if not configured
        if not term_id:
            term_id = _detect_current_term(session, headers, settings, user_id)
            if term_id:
                print(f"[step] auto-detected current term: {term_id}")
        ...
        if resp.status_code in (401, 403):
            raise AuthExpiredError(f"Auth failed ({resp.status_code}) fetching course list")
        resp.raise_for_status()
        data = resp.json().get("data", {})
        records = data.get("records", []) if isinstance(data, dict) else []
        ...
        print(f"[warn] failed to fetch course list: {exc}")
        return {}

    if not records:
        print("[warn] course list is empty")
        return {}
    ...
    for rec in records:
        if not isinstance(rec, dict):
            continue
        site_id = str(rec.get("id", rec.get("siteId", ""))).strip()
        site_name = str(rec.get("siteName", rec.get("name", ""))).strip()
        if site_id and site_name:
            courses.append((site_id, site_name))

    print(f"[step] found {len(courses)} courses, scanning assignments...")

    # Step 2: for each course, fetch its work/assignment list
    mapping: dict[str, str] = {}
    for site_id, site_name in courses:
        try:
            resp = session.post(
                _course_work_url(settings),
                headers=headers,
                json={
                    "siteId": site_id,
                    "userId": user_id,
            ...
            if resp.status_code in (401, 403):
                raise AuthExpiredError(f"Auth failed ({resp.status_code}) fetching work list")
            resp.raise_for_status()
            work_data = resp.json().get("data", {})
            work_records = work_data.get("records", []) if isinstance(work_data, dict) else []
            for work in work_records:
                if not isinstance(work, dict):
                    continue
                aid = str(work.get("activityId", work.get("id", ""))).strip()
                if aid:
                    mapping[aid] = site_name
        except AuthExpiredError:
            raise
        except Exception as exc:
            print(f"[warn] failed to fetch work list for {site_name}: {exc}")
            continue

    print(f"[ok] course map built: {len(mapping)} activity-to-course entries")
    _save_course_map(settings, mapping)
    return mapping


def _inject_course_names(
    undone_list: list[dict[str, Any]], course_map: dict[str, str],
) -> None:
    """Enrich undone items with course names from the mapping."""
    for item in undone_list:
        aid = str(item.get("activityId", item.get("id", ""))).strip()
        if aid and aid in course_map:
            item["_courseName"] = course_map[aid]
...
def _deadline_message(item: dict[str, Any], deadline: datetime, days_left: int) -> str:
    ...
    course = _pick_str(item, *_COURSE_KEYS)
    if course:
        return f"[DUE] {days_left}d left | {course} | {title} | Due: {_format_deadline(deadline)}"
    ...
    for item in undone_list:
        ...
        if known_info is None:
            state.known_assignments[aid] = {
                "name": _pick_str(item, *_TITLE_KEYS),
                "course": _pick_str(item, *_COURSE_KEYS),
            ...
            # Update course name if we now have it
            course = _pick_str(item, *_COURSE_KEYS)
            if course:
        ...
        days_left = (deadline.date() - current.date()).days
...
class Notifier:
    ...
    def _send_email(self, subject: str, message: str) -> None:
        if not self.settings.smtp_host or not self.settings.smtp_to:
            print("[warn] email skipped: SMTP_HOST or SMTP_TO is empty.")
            return
...
def run_monitor_once(*, dry_run: bool = False) -> list[str]:
    ...
    # Build course name mapping (cached, refreshed daily)
    try:
    ...
    # Enrich undone items with course names
    if course_map:
